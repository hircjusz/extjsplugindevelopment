...
Ext.ux.grid.ColumnHeaderGroup = Ext.extend(Ext.util.Observable, {
    ...
    viewConfig: {
        ...
        renderHeaders: function(){
            ...
            for(var row = 0, rlen = rows.length; row < rlen; row++){
                ...
                for(var i = 0, gcol = 0, len = r.length; i < len; i++){
                    ...
                    var cls = group.rowspan ? 'ux-grid-hd-group-cell ux-grid-hd-group-cell-rowspan' : 'ux-grid-hd-group-cell';
        ...
        renderUI: function(){
            var result = this.constructor.prototype.renderUI.apply(this, arguments);
...
Ext.ux.grid.RowEditor = Ext.extend(Ext.Panel, {
    ...
    reconfigure: function(grid, store, columnModel) {
        if (columnModel) {
            columnModel.on('hiddenchange', this.verifyLayout, this, {delay:1});
        }
        this.refreshFields();
    },
        ...
        if(this.el && (this.isVisible() || force === true)){
            ...
            for(var i = 0, len = cm.getColumnCount(); i < len; i++){
                if(!cm.isHidden(i)){
                    ...
                    //fields[i].width = cm.getColumnWidth(i) - adjust; // HACK
                } else{
                    fields[i].hide();
                    //fields[i].width = 0;
                }
    ...
    initFields: function(){
        var cm = this.grid.getColumnModel(), pm = Ext.layout.ContainerLayout.prototype.parseMargins, c, ed;
        this.removeAll(false);
        //if (this.items) {
        //    Ext.each([].concat(this.items.items), function(i) {
        //        this.remove(i, i.autoDestroy === true);
        //    }, this);
        //}

        for(var i = 0, len = cm.getColumnCount(); i < len; i++){
            c = cm.getColumnAt(i),
            ed = Ext.isFunction(c.getEditor) ? c.getEditor() : undefined;

            if(!ed){
                ed = c.displayEditor || new Ext.form.DisplayField({ autoDestroy: true });
            }
            if(i == 0){
            ...
            ed.column = c;
            if(ed.ownerCt !== this){
                ed.on('autosize', this.onAutoSize, this);
        ...
        // MOD:krwo realign dom after column change
        for (i = this.items.getCount() - 1; i >= 0; i--) {
            ed = this.items.itemAt(i);

            if (ed.getEl()) {
                var node = ed.getPositionEl().dom,
